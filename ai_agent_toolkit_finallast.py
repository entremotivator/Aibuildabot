...
from enhanced_auth_system import EnhancedAuthSystem
from admin_dashboard import AdminDashboard
from api_key_manager import APIKeyManager
from realtime_sync import RealtimeSync
from enhanced_chat_system import EnhancedChatSystem
from enhanced_database_operations import DatabaseOperationsManager
...
def chat_with_agent(user_message: str, agent_name: str, user_id: str = None) -> str:
    """Chat with an AI agent using user's own API key"""
    if not user_id:
        return "⚠️ User authentication required for chat functionality."
    
    # Get user's API key for OpenAI
    api_key_manager = APIKeyManager(st.session_state.supabase_client)
    user_api_key = api_key_manager.get_user_api_key(user_id, "OpenAI")
    
    if not user_api_key:
        return "⚠️ Please add your OpenAI API key in the API Key Management section to use chat functionality."
    
    try:
        client = OpenAI(api_key=user_api_key)
        
        all_bots = get_all_bots(user_id)
        ...
        chat_history = st.session_state.enhanced_chat.get_chat_history(user_id, agent_name, limit=6)
        ...
        # Save to database
        st.session_state.enhanced_chat.save_chat_message(
            user_id, agent_name, user_message, assistant_response
        )
        
        # Update API usage tracking
        api_key_manager.update_api_usage(user_id, "OpenAI", response.usage.total_tokens)
        ...
        if "invalid api key" in str(e).lower():
            return "⚠️ Invalid OpenAI API key. Please check your API key in the API Key Management section."
...
# auth = simple_auth() # This is replaced by EnhancedAuthSystem
...
def init_session_state():
    ...
    if 'supabase_client' not in st.session_state:
        st.session_state.supabase_client = EnhancedSupabaseClient()
    if 'auth_system' not in st.session_state:
        st.session_state.auth_system = EnhancedAuthSystem(st.session_state.supabase_client)
    if 'realtime_sync' not in st.session_state:
        st.session_state.realtime_sync = RealtimeSync(st.session_state.supabase_client)
    if 'enhanced_chat' not in st.session_state:
        st.session_state.enhanced_chat = EnhancedChatSystem(st.session_state.supabase_client)
    if 'db_operations' not in st.session_state:
        st.session_state.db_operations = DatabaseOperationsManager(st.session_state.supabase_client)
...
def display_sidebar():
    """Display enhanced sidebar with navigation and user info"""
    with st.sidebar:
        # User info section
        st.markdown(f"""
        <div class="user-info-card">
            ...
            <p><strong>Role:</strong> {'Admin' if st.session_state.is_admin else 'User'}</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Main pages
        pages = ["💬 Chat", "🛠️ Manage Custom Bots", "👤 User Profile", "🔑 API Keys"]
        
        # Add admin pages
        if st.session_state.is_admin:
            pages.extend(["🛡️ Admin Dashboard", "🗄️ Database Operations"])
        else:
            pages.append("📊 My Analytics")
        ...
        if st.session_state.current_page == "Chat":
            ...
            for bot_name, bot_data in all_bots.items():
                ...
                if category not in categories:
                    categories[category] = []
                categories[category].append(bot_name)
            
            # Display bots by category
            for category, bots in categories.items():
                st.markdown(f"**{category}**")
                for bot_name in bots:
                    bot_data = all_bots[bot_name]
                    is_selected = st.session_state.selected_agent == bot_name
                    
                    if st.button(
                        f"{bot_data.get('emoji', '🤖')} {bot_name}",
                        key=f"select_{bot_name}",
        ...
        st.session_state.realtime_sync.render_notifications_sidebar(st.session_state.user_id)
        
        st.divider()
        
        # Logout button
        if st.button("🚪 Logout", type="secondary", use_container_width=True):
            st.session_state.auth_system.logout()
            st.rerun()
...
# def login_form(): # This is replaced by EnhancedAuthSystem
...
def display_chat_page():
    ...
    <div class="main-header">
        ...
        <p>Chat with specialized AI agents using your own API keys</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Render enhanced chat system
    st.session_state.enhanced_chat.render_chat_interface(
        st.session_state.user_id, 
        st.session_state.selected_agent
    )

def display_api_keys_page():
    """Display API key management page"""
    st.markdown("""
    <div class="main-header">
        <h1>🔑 API Key Management</h1>
        <p>Manage your API keys for different AI providers</p>
    </div>
    """, unsafe_allow_html=True)
    
    api_key_manager = APIKeyManager(st.session_state.supabase_client)
    api_key_manager.render_api_key_management(st.session_state.user_id)

def display_admin_dashboard_page():
    ...
    admin_dashboard = AdminDashboard(st.session_state.supabase_client)
    admin_dashboard.render_admin_dashboard()

def display_database_operations_page():
    """Display database operations page"""
    if not st.session_state.is_admin:
        st.error("Access denied. Admin privileges required.")
        return
    ...
    st.markdown("""
    <div class="main-header">
        <h1>🗄️ Database Operations</h1>
        <p>Advanced database management and analytics</p>
    </div>
    """, unsafe_allow_html=True)
    
    st.session_state.db_operations.render_database_operations(
        st.session_state.user_id, 
        st.session_state.is_admin
    )

def display_my_analytics_page():
    """Display user analytics page"""
    st.markdown("""
    <div class="main-header">
        <h1>📊 My Analytics</h1>
        <p>Your personal usage statistics and insights</p>
    </div>
    """, unsafe_allow_html=True)
...
def display_chat_page_old(): # Renamed to avoid conflict with enhanced version
...
def main():
    ...
    if not st.session_state.authenticated:
        ...
        st.session_state.auth_system.render_auth_interface()
    else:
        # Display sidebar
        display_sidebar()
        ...
        page_routing = {
            "Chat": display_chat_page,
            "Manage Custom Bots": display_custom_bots_page,
            "User Profile": display_user_profile_page,
            "API Keys": display_api_keys_page,
            "Admin Dashboard": display_admin_dashboard_page,
            "Database Operations": display_database_operations_page,
            "My Analytics": display_my_analytics_page
        }
        ...
        # Render real-time sync components
        st.session_state.realtime_sync.render_realtime_components(st.session_state.user_id)
